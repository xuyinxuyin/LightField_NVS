...
def read_image(path):
     im_array= cv2.imread(path,1)
     im_array= np.stack([im_array[...,2],im_array[...,1], im_array[...,0]],axis=-1)/255.
     im_array= im_array[None,...]
     return im_array

if __name__ == '__main__':

    path = '/home/ubuntu/IBRNet/eval/multishapenet/eval_rebuttal'
    ...
    for root, dirs, files in os.walk(path, topdown=False):
        for name in dirs:
            scene_list.append(os.path.join(root, name))
    #scene_list = glob.glob(path)

    total_num = len(scene_list)
    
    
    tf.compat.v1.disable_eager_execution()
    pred_ph = tf.compat.v1.placeholder(tf.float32)
    gt_ph = tf.compat.v1.placeholder(tf.float32)
    ...
    for scene in scene_list: 
        ...
        gt_file_list = glob.glob(scene+'/*_gt_rgb.png')
        gt_rgb_np = read_image(gt_file_list[0])
        pred_file_list = glob.glob(scene+'/*_pred_fine.png')
        fine_pred_rgb_np = read_image(pred_file_list[0])
        ...
        sum_fine_psnr =sum_fine_psnr+fine_psnr
        sum_fine_lpips =sum_fine_lpips+fine_psnr
        sum_fine_ssim = sum_fine_ssim+fine_ssim
            
    print(sum_fine_psnr/total_num)
    print(sum_fine_ssim/total_num)
    print(sum_fine_lpips/total_num)
